...
class Menu():
    ...
    def __init__(self, title: str, items: list[dict], window: any, position: str = 'center', text_alignment: str = 'center') -> None:
        ...
        labels.append(title)
        self.horizontal_length = int(len(max(labels, key=len)))
        labels.remove(title)
        try:
        ...
        self.window_border.addstr(0, (self.window_border.getmaxyx()[1] - len(title)) // 2, title)
        ...
        while True:
            ...
            for idx, item in enumerate(self.items):
                ...
                if self.text_alignment == 'center':
                    label = ' ' * ((self.window.getmaxyx()[1] - len(label)) // 2 - left_spacing) + label + ' ' * ((self.window.getmaxyx()[1] - len(label)) // 2 - right_spacing)
